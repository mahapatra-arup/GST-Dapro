...
namespace DAPRO.SalesFolder.Invoice
{
    public class CDNURFormate
    {
        ...
        public static DataTable CDNURnvoiceGenerate(string startDate, string endDate)
        {
            string stateCode = "";
            InitialColumn();
            ...
            mDt.Rows.Add("Summary For CDNUR(9B)");
            mDt.Rows.Add("", "No. of Notes/Vouchers", "","", "No. of Invoices","", "", "","Total Note Value", "", "Total Taxable Value", "Total Cess");
            mDt.Rows.Add();
            mDt.Rows.Add("UR Type", "Note/Refund Voucher Number", "Note/Refund Voucher date", "Document Type", "Invoice/Advance Receipt Number", "Invoice / Advance Receipt date",
            "Reason For Issuing document", "Place Of Supply", "Note/Refund Voucher Value", "Rate", "Taxable Value", "Cess Amount", "Pre GST");

           
            #region sINGLE
            string qry = "DECLARE @STATECODE NVARCHAR(200)  " +
           "SET @STATECODE=(SELECT TOP(1)StateID FROM OrganizationDetails)  ";
            //Condition
            string condition = "(ISNULL(CDRNote.GSTIN, '') = '') AND(CDRNote.RefundDate BETWEEN '" + startDate + "' and '" + endDate + "')   and CDRNote.InvoiceValue>250000 and LEFT(CDRNote.PlaceOfSupply, Charindex('-', CDRNote.PlaceOfSupply) -1)!='" + qry + "'  ";
            //
            ...
            if (dt.IsValidDataTable())
            {
                foreach (DataRow item in dt.Rows)
                {
                    ...
                    //Split State Code
                    string str = item["PlaceOfSupply"].ToString();
                    string[] splittedStr = str.Split('-');
                    stateCode = splittedStr[0].ToString().Trim();

                    mDt.Rows.Add("B2CL", item["NoteNo"], item["RefundDate"], item["DocumentType"], item["InvoiceOrADRNo"], item["InvoiceOrADRDate"], item["Reason"], item["PlaceOfSupply"]
            ...
            #region TOTAL
            //Get Total
            string totamount = "(select sum(Inv.Notevalue)  from (select  InvoiceOrADRNo,sum(NoteValue) as  Notevalue from  CDRNote where " + condition + "  group by InvoiceOrADRNo) as Inv) as TotalNoteValue";

            query = "SELECT   "+ totamount + "  ,COUNT(Distinct CDRNote.NoteNo) AS CountNoteNo, COUNT(Distinct CDRNote.InvoiceOrADRNo) AS CountInvoiceOrADRNo,  " +
            ...
            if (dt1.IsValidDataTable())
            {
                ...
                mDt.Rows[2][1] = dt1.Rows[0]["CountNoteNo"];
                mDt.Rows[2][4] = dt1.Rows[0]["CountInvoiceOrADRNo"];
